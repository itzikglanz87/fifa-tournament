...
**רץ לבד:** משימה מתוזמנת ב‑Windows בשם "FIFA score camera" מריצה אותו בכל
כניסה למשתמש, מוסתר, עם `--log` (הפלט ל‑`tools/score_cam.log`). כל עוד אין
משחק חי הוא ישן ולא פותח את המצלמה; המתג "📷 קריאה מהמצלמה" בלשונית ניהול
מדליק ומכבה אותו מהטלפון.
